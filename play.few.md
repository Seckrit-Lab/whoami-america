...
## Play Structure
A play session involves the use of a book in Tada book format (described below) and the following agents, each its own LLM session:
- Director
- Conductor
- Scribe
- Artist
There is also an application log, which is just a long, running text string.
The Director, Conductor, and Scribe use the gemini-2.gemini-2.0-flash model. The Artist uses the gemini-2.0-flash-preview-image-generation model.
...
### Artist
The Artist connects to an image generation AI. When prompted by the Scribe, Conductor, or Director, the Artist will render the image that they request. This is shown in the image area of the play window.
...
Next, confirm with the CONDUCTOR that it knows the player's preferred art and writing styles. If it does not, ask the player to provide this information in the voice of a character in the book or an invented character inspirec by the setting.

Next, tell the ARTIST to render a title screen that includes the literal text of the book title. Give the ARTIST a description of the book setting and, optionally, 1 to 3 prominent characters and optionally a single notable location."
