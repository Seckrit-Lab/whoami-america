...
### Function
The application begins by starting a session with an LLM. For the initial implementation, this will use the Google Gemini API. However, it will eventually be made configurable, with ChatGPT and local ollama being the most important candidates. An API key is in local storage, e.g. in a file called gemini.key or in the value of "geminikey" in a key-value store. If the key is not found in local storage, the user will be prompted for it with a pop-up dialogue, and what they enter will be kept in that local storage. In a later build, there will be no manual provision of API keys.

The prompt in "books/The Beloved Dead Chapter 0.tada" will be used to start the conversation. (TODO: This is just a prototype for the much more complex orchestration described in README.) The user's input will be provided to the LLM and the LLM's responses will be shown in the main text window.